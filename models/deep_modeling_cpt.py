...
from models.deep_modeling_cpt_utils import BertModel
...
class CPTForMaskedLM(CPTPretrainedModel):
    ...
    def __init__(self, config, n_prompt_tokens, **kwargs):
        super().__init__(config)
        self.model = CPTModel(config)
    ...
    def forward(
        ...
        enc_logits = self.lm_head(enc_hidden_states[torch.arange(input_ids.size(0)), mask_pos])
