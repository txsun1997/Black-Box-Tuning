...
class ElectraModel(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        if prompt_embedding is not None:
            ...
            prompt_padding = torch.zeros(bsz, input_shape[1] - n_prompt_tokens - 1, prompt_dim).to(device)
            extended_prompt_embedding = torch.cat([prompt_embedding, prompt_padding], dim=1)
            pre_padding = torch.zeros(bsz, 1, prompt_dim).to(device)
...
class ElectraForMaskedLM(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        return MaskedLMOutput(
            loss=loss,
            logits=prediction_scores[torch.arange(prediction_scores.size(0)), mask_pos],
