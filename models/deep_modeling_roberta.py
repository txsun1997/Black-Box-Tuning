...
class RobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        bsz, seq_len, prompt_dim = hidden_states.shape
        device = hidden_states.device
        prompt_padding = torch.zeros(
            bsz, seq_len - prompt_embedding.size(1) - 1, prompt_dim, device=device)
        pre_padding = torch.zeros(bsz, 1, prompt_dim, device=device)

        for i, layer_module in enumerate(self.layer):
            hidden_states = hidden_states + torch.cat([pre_padding,
                                                       prompt_embedding[i].to(device).reshape(prompt_embedding.size(1),
                                                                                              prompt_dim).repeat(bsz, 1,
                                                                                                                 1),
                                                       prompt_padding], dim=1)
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if concat_prompt and prompt_embedding is not None:
            n_prompt_tokens = prompt_embedding.shape[1]
            attention_mask_padding = torch.ones(
                input_shape[0], n_prompt_tokens).to(attention_mask.device)
            attention_mask = torch.cat(
                [attention_mask_padding, attention_mask], 1).to(input_ids.device)
    ...
    def forward(
        ...
        if self.inference_framework == 'pt':
            ...
            return MaskedLMOutput(
                loss=masked_lm_loss,
                logits=prediction_scores[torch.arange(
                    sequence_output.size(0)), mask_pos],
                hidden_states=outputs.hidden_states,
                attentions=outputs.attentions,
            )
            ...
            outputs = torch.empty(
                (bsz, max_seq_len, self.config.hidden_size), dtype=torch.float32, device='cuda')
            ...
            io_binding.bind_input(name='input_ids', device_type='cuda', device_id=0, element_type=np.longlong,
                                  shape=(bsz, max_seq_len), buffer_ptr=input_ids.data_ptr())
            ...
            io_binding.bind_input(name='prompt_embedding', device_type='cuda', device_id=0, element_type=np.float32,
                                  shape=(self.config.num_hidden_layers,
                                         self.n_prompt_tokens, self.config.hidden_size),
                                  buffer_ptr=prompt_embedding.data_ptr())
            io_binding.bind_output('logits', device_type='cuda', device_id=0, element_type=np.float32,
                                   shape=outputs.shape, buffer_ptr=outputs.data_ptr())
...
class RobertaForPromptTuning(RobertaPreTrainedModel):
    ...
    def __init__(self, config, n_prompt_tokens):
        ...
        self.prompt_embedding = nn.Embedding(
            n_prompt_tokens, config.hidden_size)
    ...
    def forward(
        ...
        prompt_embedding = self.prompt_embedding(
            torch.arange(0, self.n_prompt_tokens).to(input_ids.device))
        ...
        sequence_output = outputs[0]
        sequence_mask_output = sequence_output[torch.arange(
            sequence_output.size(0)), mask_pos]
